from waveport.util_lfbd import load_mesh, Edge, quad_eval, quad_sample_points, where, plot_csv
import numpy as np
...
class Waveguide3DLFBD:
    ...
    def __init__(self, filename, k0, volume_names, volume_permittivities, pec_name, abc_name, ip_surface_names, ip_boundary_name, ip_permittivities, op_surface_names, op_boundary_name, op_permittivities, p1_ip=None, p2_ip=None, p1_op=None, p2_op=None):
        ...
        self.K1 = np.zeros([len(self.remap_edge_nums), len(self.remap_edge_nums)], dtype=complex)
        self.K2 = np.zeros([len(self.remap_edge_nums), len(self.remap_edge_nums)], dtype=complex)
        self.KEN = np.zeros([len(self.remap_edge_nums), len(self.remap_node_nums)], dtype=complex)
        self.KNE = np.zeros([len(self.remap_node_nums), len(self.remap_edge_nums)], dtype=complex)
        self.GNN = np.zeros([len(self.remap_node_nums), len(self.remap_node_nums)], dtype=complex)
    ...
    def construct_simple(filename, k0=4., permittivity=1., p1_ip=None, p2_ip=None, p1_op=None, p2_op=None):
        ...
        output port. Default = None.
        """
        return Waveguide3DLFBD(filename, k0, ["Tetrahedrons"], [permittivity], "PECWalls", None, ["InputPort"], "InPortBoundary", [permittivity], ["OutputPort"], "OutPortBoundary", [permittivity], p1_ip, p2_ip, p1_op, p2_op)
    ...
    def solve(self, mode=0):
        ...
        for tet in self.tetrahedrons:
            ...
            for edgei in tet.edges:
                ...
                for index, nodej in enumerate(tet.nodes):
                    if nodej in self.boundary_pec_node_numbers:
                        continue
                    bj, cj, dj = tet.simplex_consts[index, 1:4]
                    I1 = bj*Axl + cj*Ayl + dj*Azl
                    I2 = (cj*Byl + dj*Bzl) * x_mean
                    I3 = (bj*Bxl + dj*Czl) * y_mean
                    I4 = (bj*Cxl + cj*Cyl) * z_mean
                    i_sum = I1 + I2 + I3 + I4
                    self.KEN[self.remap_edge_nums[edgei], self.remap_node_nums[nodej]] += tet.permittivity * edge1.length / 216 / tet.volume**2 * i_sum
                ...
                for edgej in tet.edges:
                    ...
                    if edgei in self.boundary_input_edge_numbers and edgej in self.boundary_input_edge_numbers:
                        ...
                        if found_edge_no != -1:
                            ...
                            self.K1[self.remap_edge_nums[edgei], self.remap_edge_nums[edgej]] += integral * self.input_port.get_selected_beta() * 1j
                    ...
                    # If working with output port edges, need to do different integral
                        ...
                        if found_edge_no != -1:
                            ...
                            # ni_dot_nj = np.reshape(N_i[:, 0] * N_j[:, 0] + N_i[:, 1] * N_j[:, 1] + N_i[:, 2] * N_j[:, 2], [len(sample_points), 1])
                            integral = quad_eval(nodes[0], nodes[1], nodes[2], dotted)
                            self.K1[self.remap_edge_nums[edgei], self.remap_edge_nums[edgej]] += self.output_port.get_selected_beta() * integral * 1j
                        ...
                        if found_edge_no != -1:
                            ...
                            # ni_dot_nj = np.reshape(N_i[:, 0] * N_j[:, 0] + N_i[:, 1] * N_j[:, 1] + N_i[:, 2] * N_j[:, 2], [len(sample_points), 1])
                            integral = quad_eval(nodes[0], nodes[1], nodes[2], dotted)
                            self.K1[self.remap_edge_nums[edgei], self.remap_edge_nums[edgej]] += self.k0 * integral * 1j
                            continue
                    ...
                    self.K1[index1, index2] += curl_dot_curl_part
                    # TODO: Evaluate if I should be replacing k0**2 with \omega**2
                    self.K2[index1, index2] += tet.permittivity * dot_part
            # Back to tetrahedron loop. Iterate over nodes first now
            for i, nodei in enumerate(tet.nodes):
                # Skip PEC nodes
                if nodei in self.boundary_pec_node_numbers:
                    continue
                ai, bi, ci, di = tet.simplex_consts[i]
                # Iterate over the nodes to compute GNN
                for j, nodej in enumerate(tet.nodes):
                    # SKip PEC nodes
                    if nodej in self.boundary_pec_node_numbers:
                        continue
                    aj, bj, cj, dj = tet.simplex_consts[j]
                    I1 = ai*aj
                    I2 = (ai*bj + aj*bi) * x_mean
                    I3 = (ai*cj + aj*ci) * y_mean
                    I4 = (ai*dj + aj*di) * z_mean
                    I5 = 1/20 * (bi*cj + bj*ci) * (sum(xi*yi for xi, yi, zi in tet.points) + 16*x_mean*y_mean)
                    I6 = 1/20 * (ci*dj + cj*di) * (sum(yi*zi for xi, yi, zi in tet.points) + 16*y_mean*z_mean)
                    I7 = 1/20 * (bi*dj + bj*di) * (sum(xi*zi for xi, yi, zi in tet.points) + 16*x_mean*z_mean)
                    I8 = 1/20 * (bi*bj) * (sum(xi*xi for xi, yi, zi in tet.points) + 16*x_mean*x_mean)
                    I9 = 1/20 * (ci*cj) * (sum(yi*yi for xi, yi, zi in tet.points) + 16*y_mean*y_mean)
                    I10 = 1/20 * (di*dj) * (sum(zi*zi for xi, yi, zi in tet.points) + 16*z_mean*z_mean)
                    i_sum = I1 + I2 + I3 + I4 + I5 + I6 + I7 + I8 + I9 + I10
                    self.GNN[self.remap_node_nums[nodei], self.remap_node_nums[nodej]] += i_sum / 36 / tet.volume
                # Iterate over the edges to compute KNE
                for edgej in tet.edges:
                    # Skip over PEC walls
                    if edgej in self.boundary_pec_edge_numbers:
                        continue
                    ...
                    Bzk = a_ik[1] * a_jk[3] - a_ik[3] * a_jk[1]
                    Czk = a_ik[2] * a_jk[3] - a_ik[3] * a_jk[2]
                    if edgej in self.boundary_input_edge_numbers:
                        # Need the third point that makes up the face of the input port
                        found_edge_no = -1
                        # Search for a different edge on the input port from this tetrahedral element
                        for edge in tet.edges:
                        ...
                        if found_edge_no != -1:
                            ...
                            v2 = nodes[2] - nodes[0]
                            # We SHOULD find the n_hat that points out of the surface, but inward pointing will cancel anyway
                            # TODO: This above does matter for this case. For now, using z_hat as n_hat.
                            # n_hat = np.cross(v1, v2)
                            ...
                            N_j[:, 2] = Azk + Bzk * sample_points[:, 0] + Czk * sample_points[:, 1]
                            N_j = N_j * edge2.length / 36 / tet.volume**2
                            lam = ai + bi*sample_points[:, 0] + ci*sample_points[:, 1] + di*sample_points[:, 2]
                            n_hat_dot_nj = np.array([np.dot(n_hat, vec) for vec in N_j])
                            # TODO: Use the above properly (not being used at all right now for some reason)
                            # dotted = n_hat_x_ni[:, 0] * n_hat_x_nj[:, 0] + n_hat_x_ni[:, 1] * n_hat_x_nj[:, 1] + n_hat_x_ni[:, 2] * n_hat_x_nj[:, 2]
                            # ni_dot_nj = np.reshape(N_i[:, 0] * N_j[:, 0] + N_i[:, 1] * N_j[:, 1] + N_i[:, 2] * N_j[:, 2], [len(sample_points), 1])
                            values = lam * n_hat_dot_nj
                            values = np.reshape(values, [len(values), 1])
                            # ni_dot_nj = N_i[:, 0] * N_j[:, 0] + N_i[:, 1] * N_j[:, 1] + N_i[:, 2] * N_j[:, 2]
                            integral = quad_eval(nodes[0], nodes[1], nodes[2], values)
                            self.KNE[self.remap_node_nums[nodei], self.remap_edge_nums[edgej]] += tet.permittivity * integral
                    elif edgej in self.boundary_output_edge_numbers:
                        # Need the third point that makes up the face of the input port
                        found_edge_no = -1
                        # Search for a different edge on the input port from this tetrahedral element
                        for edge in tet.edges:
                    ...
                    I1 = bi*Axk + ci*Ayk + di*Azk
                    I2 = (ci*Byk + di*Bzk) * x_mean
                    I3 = (bi*Bxk + di*Czk) * y_mean
                    I4 = (bi*Cxk + ci*Cyk) * z_mean
                    i_sum = I1 + I2 + I3 + I4
                    self.KNE[self.remap_node_nums[nodei], self.remap_edge_nums[edgej]] += -tet.permittivity * edge2.length / 216 / tet.volume**2 * i_sum

        # Need to invert GNN
        GNN_inv = inv(self.GNN)
        ...
        self.K = self.K1 - self.k0**2*self.K2 - (self.KEN @ GNN_inv @ self.KNE)
