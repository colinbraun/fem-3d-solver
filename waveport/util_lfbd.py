...
def load_mesh(filename, volume_names, volume_permittivities, pec_name, abc_name, ip_surface_names, ip_boundary_name, ip_permittivities, op_surface_names, op_boundary_name, op_permittivities):
    ...
    if pec_name is None:
        ...
        boundary_pec_node_numbers = set()
        for edge in boundary_pec_edges:
            boundary_pec_node_numbers.add(edge.node1)
            boundary_pec_node_numbers.add(edge.node2)
    ...
    # Get the set of non-PEC global node numbers
    node_nums = set(np.arange(0, len(all_nodes))) - boundary_pec_node_numbers
    # A map that takes one of the non-PEC node numbers and maps it to a unique integer between [0, num of non-PEC nodes]
    remap_node_nums = {item: i for i, item in enumerate(node_nums)}
    ...
    return all_nodes, all_tets, tets_node_ids, all_edges, boundary_pec_node_numbers, boundary_pec_edge_numbers, abc_edge_numbers, boundary_input_edge_numbers, boundary_output_edge_numbers, remap_node_nums, remap_edge_nums, all_edges_map, boundary_input_tets, boundary_output_tets
