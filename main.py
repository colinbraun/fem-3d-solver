...
all_nodes, tetrahedrons, tets_node_ids, all_edges, boundary_pec_edge_numbers, boundary_input_edge_numbers, boundary_output_edge_numbers, remap_edge_nums, all_edges_map, boundary_input_triangles, boundary_output_triangles, boundary_input_tets, boundary_output_tets = load_mesh("rectangular_waveguide_3d_less_coarse.inp")
...
for tet in tetrahedrons:
    ...
    for edgei in tet.edges:
        ...
        if tet in boundary_input_tets:
            ...
            if len(found_edge_nos) == 2:
                found_edge1 = all_edges[found_edge_nos[0]]
                found_edge2 = all_edges[found_edge_nos[1]]
                nodes = np.unique(np.array([all_nodes[found_edge1.node1], all_nodes[found_edge1.node2], all_nodes[found_edge2.node1], all_nodes[found_edge2.node2]]), axis=0)
                if len(nodes) != 3:
                ...
                E_inc = np.array([np.array(waveguide.get_field_at(sample_point[0], sample_point[1])) for sample_point in sample_points])
        ...
        for edgej in tet.edges:
            ...
            node_ik, node_jk = all_nodes[edge2.node1], all_nodes[edge2.node2]
            ...
            if edgei in boundary_input_edge_numbers and edgej in boundary_input_edge_numbers:
                ...
                if found_edge_no != -1:
                    ...
                    nodes = np.unique(
                        np.array([node_ik, node_jk, all_nodes[found_edge.node1], all_nodes[found_edge.node2]]), axis=0)
                    if len(nodes) != 3:
                        raise RuntimeError("Did not find 3 nodes on surface triangle")
                    ...
                    K[remap_edge_nums[all_edges_map[edge1]], remap_edge_nums[all_edges_map[edge2]]] += integral * beta * 1j
            ...
            # If working with output port edges, need to do different integral
                ...
                for edge in tet.edges:
                    if edge == edgej:
                        continue
                    elif edge in boundary_output_edge_numbers:
                        # We found an edge containing the third node, note it
                        found_edge_no = edge
                        break
                # It is possible we do not find another edge that lies on the input port. We are checking for this here.
                if found_edge_no != -1:
                    ...
                    ni_dot_nj = np.reshape(N_i[:, 0] * N_j[:, 0] + N_i[:, 1] * N_j[:, 1] + N_i[:, 2] * N_j[:, 2], [len(sample_points), 1])
                    integral = quad_eval(nodes[0], nodes[1], nodes[2], ni_dot_nj)
                    K[remap_edge_nums[all_edges_map[edge1]], remap_edge_nums[all_edges_map[edge2]]] += beta * integral * 1j
                pass
...
for i, tet_index in enumerate(tet_indices):
    ...
    Ex[y_i, x_i, z_i], Ey[y_i, x_i, z_i], Ez[y_i, x_i, z_i] = np.abs(tet.interpolate(phis, field_points[i]))
    # Ex[y_i, x_i, z_i] = abs(ex)
    # Ey[y_i, x_i, z_i] = abs(ey)
    # Ez[y_i, x_i, z_i] = abs(ez)
